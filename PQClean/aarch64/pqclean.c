...
/*
 * Compute the signature in Public-Key-Recovery (PKR) mode. nonce[] receives the nonce and must have length
 * NONCELEN bytes. sigbuf[] receives the signature value (without nonce
 * or header byte), with *sigbuflen providing the maximum value length and
 * receiving the actual value length.
 *
 ...
 */
static int
do_sign_pkr(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
        const uint8_t *m, size_t mlen, const uint8_t *sk) {
    union {
        uint8_t b[72 * FALCON_N];
        uint64_t dummy_u64;
     ...
     * Compute and return the signature.
     */
    PQCLEAN_FALCON512_AARCH64_sign_dyn_pkr(r.sig, &sc, f, g, F, G, r.hm, tmp.b);
    v = PQCLEAN_FALCON512_AARCH64_comp_encode_pkr(sigbuf, *sigbuflen, r.sig);
    if (v != 0) {
        inner_shake256_ctx_release(&sc);
        *sigbuflen = v;
        return 0;
...
/*
 * Verify a signature in Public-Key-Recovery (PKR) mode and recover the public key from it.
 * The nonce has size NONCELEN bytes.
 * sigbuf[] (of size sigbuflen) contains the signature value, not including the
 * header byte or nonce. Return value is 0 on success, -1 on error.
 */
static int
do_verify_pkr_recover(
    const uint8_t *nonce, const uint8_t *sigbuf, size_t sigbuflen,
    const uint8_t *m, size_t mlen, uint8_t *pk) {
    union {
        uint8_t b[2 * FALCON_N];
        uint64_t dummy_u64;
        fpr dummy_fpr;
    ...
    v = PQCLEAN_FALCON512_AARCH64_comp_decode_pkr(sig, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
        if (sigbuflen == PQCLEAN_FALCONPADDED512_AARCH64_CRYPTO_BYTES_PKR - NONCELEN - 1) {
            while (v < sigbuflen) {
                if (sigbuf[v++] != 0) {
                    return -1;
                }
    ...
    if (!PQCLEAN_FALCON512_AARCH64_verify_recover(h, hm, sig + 512, sig, tmp.b)) {
        return -1;
    }
    PQCLEAN_FALCON512_AARCH64_poly_convert_to_unsigned(h);
    uint16_t h_unsigned[FALCON_N];
    memcpy(h_unsigned, h, FALCON_N * sizeof * h_unsigned);
...
/* see api.h */
int
PQCLEAN_FALCON512_AARCH64_crypto_sign_verify_pkr_recover(
    const uint8_t *sig, size_t siglen,
    const uint8_t *m, size_t mlen, uint8_t *pk) {
    if (siglen < 1 + NONCELEN) {
        return -1;
    ...
    return do_verify_pkr_recover(sig + 1,
                     sig + 1 + NONCELEN, siglen - 1 - NONCELEN, m, mlen, pk);
}

/* see api.h */
int
PQCLEAN_FALCON512_AARCH64_crypto_sign_signature_pkr(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;

    vlen = PQCLEAN_FALCON512_AARCH64_CRYPTO_BYTES_PKR - NONCELEN - 1;
    if (do_sign_pkr(sig + 1, sig + 1 + NONCELEN, &vlen, m, mlen, sk) < 0) {
        return -1;
    }
    sig[0] = 0x30 + FALCON_LOGN;
    *siglen = 1 + NONCELEN + vlen;
