...
#define PQCLEAN_FALCON512_AARCH64_CRYPTO_BYTES_PKR        1463
...
#define PQCLEAN_FALCONPADDED512_AARCH64_CRYPTO_BYTES_PKR  1283 // used in signature verification
 ...
 * (in bytes) is PQCLEAN_FALCON512_AARCH64_CRYPTO_BYTES.
 *
...
/*
 * Compute a signature on a provided message (m, mlen), with a given
 * private key (sk) in Public-Key-Recovery (PKR) mode. Signature is written in sig[], with length written
 * into *siglen. Signature length is variable; maximum signature length
 * (in bytes) is PQCLEAN_FALCON512_AARCH64_CRYPTO_BYTES_PKR.
 *
 * sig[], m[] and sk[] may overlap each other arbitrarily.
 *
 * Return value: 0 on success, -1 on error.
 */
int PQCLEAN_FALCON512_AARCH64_crypto_sign_signature_pkr(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk);
...
/*
 * Verify a signature (sig, siglen) in Public-Key-Recovery (PKR) mode on a message (m, mlen)
 * and recovery the public key from the signature.
 *
 * sig[], m[] and pk[] may overlap each other arbitrarily.
 *
 * Return value: 0 on success, -1 on error.
 */
int PQCLEAN_FALCON512_AARCH64_crypto_sign_verify_pkr_recover(
    const uint8_t *sig, size_t siglen,
 ...
 * (mlen) by up to PQCLEAN_FALCON512_AARCH64_CRYPTO_BYTES.
 *
 ...
 * may range up to PQCLEAN_FALCON512_AARCH64_CRYPTO_BYTES.
 *
