...
"""
# Enables the library to provide more hash algorithms like XOF hash algorithms
# featured in PyCryptodome
...
XOF_hash_functions = [SHAKE256, TurboSHAKE256, cSHAKE256, KangarooTwelve]
...
class VerifyingKey(object):
    ...
    def __repr__(self):
        ...
        if self.default_hashfunc and XOF_hash_functions.count(self.default_hashfunc) == 0:
    ...
    def from_public_key_recovery(
        ...
        # Only one of the following two lines should be not commented out.
        # This is necessary to either enable the already supplied hash algorithms (line 1)
        # or enable the XOF hash algorithms of PyCryptodome (line 2).
        if XOF_hash_functions.count(hashfunc) == 0:
            digest = hashfunc(data).digest()
        else:
            digest = hashfunc.new().update(data).read(32)
...
class SigningKey(object):
    ...
    def sign(
        ...
        if XOF_hash_functions.count(hashfunc) == 0:
            ...
            h = hashfunc.new().update(data).read(32)
